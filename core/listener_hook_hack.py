...
def enable_dictation_mode():
    ...
    if not dictation_mode_in_use:
        ...
        talon_actions.user.gdb_disable()
        sleep_mode_in_use = False
        command_mode_in_use = False
...
def listen_for_commands(server_socket: socket.socket, logger: logging.Logger):
    while True:
        time.sleep(1)
        ...
        with connection:
            ...
            if len(data) > 0:
                if data == b'c':
                    logger.info(f'Received {data}. Command mode')
                    enable_command_mode()
                    # send_gnome_msg(send_gnome_msg_title, 'Command', timeout_sec)
                elif data == b'd':
                    logger.info(f'Received {data}. Dictation mode')
                    enable_dictation_mode()
                    # send_gnome_msg(send_gnome_msg_title, 'Dictation', timeout_sec)
                elif data == b's':
                    logger.info(f'Received {data}. Sleep mode')
                    enable_sleep_mode()
                    # send_gnome_msg(send_gnome_msg_title, 'Sleep', timeout_sec)
                else:
                    logger.info(f"Received unknown command: {data}")


def is_socket_closed(connection: socket.socket, logger: logging.Logger) -> bool:
    try:
        # this will try to read bytes without blocking and also without removing them from buffer (peek only)
        connection.recv(16, socket.MSG_DONTWAIT | socket.MSG_PEEK)
...
class CommandListener(threading.Thread):
    ...
    def run(self):
        ...
        listen_for_commands(self.server_socket, self.logger)

    # Deleting (Calling destructor)
    def __del__(self):
        self.server_socket.close()
        self.logger.info(f"Closed {self.name}")
